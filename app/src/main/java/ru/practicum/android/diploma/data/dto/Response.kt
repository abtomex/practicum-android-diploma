...
sealed class Response<T> (val resultCode: Int, val body: T?) {
    ...
    ) : Response<List<IndustryDto>>(SUCCESS_RESPONSE_CODE, industries)
    ...
    class VacancyDetailsResponse(
        vacancyDetails: VacancyDetailsDto
    ) : Response<VacancyDetailsDto>(SUCCESS_RESPONSE_CODE, vacancyDetails)
